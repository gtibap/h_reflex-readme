...
def signal_segmentation(name_title):
    global df_max, fig_seg, ax_seg, obj_list
    ...
    for tx in df_sel.iloc[:,ch_time]:
        ...
        df_roi = df[(df.iloc[:,ch_time]>=(tx-0.003)) & (df.iloc[:,ch_time]<(tx+0.004))]
        # print(f"df_roi:\n{df_roi}")
        ## peak stimulation
...
#########################
def on_press(event):
    global ax_seg, fig_seg, tmin, tmax, obj_list
    ...
    ## measuring max. amplitude stimulation voltage
        ...
        # print(f"\n")
        print(f"max value: {np.max(vpp_list)}, stim: {np.argmax(vpp_list)}")
        print(f"\n")
